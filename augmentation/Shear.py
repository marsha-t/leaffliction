...
def shear(image: Image.Image, shear_factor: float, horizontal: bool) -> Image.Image:
    """
    ...
    if horizontal:
        return image.transform(image.size, Image.Transform.AFFINE, data=(1, shear_factor, 0, 0, 1, 0))
    else:
        return image.transform(image.size, Image.Transform.AFFINE, data=(1, 0, 0, shear_factor, 1, 0))
